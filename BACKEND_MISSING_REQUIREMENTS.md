...
public class User {
    ...
    @Column(nullable = false)
    private Integer highestUnlockedLevel = 5; // Match Card Game: First 5 levels unlocked by default

    @Column(nullable = false)
    private Integer highestUnlockedPatternLevel = 5; // Pattern Matching Game: First 5 levels unlocked by default
}
```
...
### D. Game 1: Match The Card Telemetry & Completion

- **Method**: `POST`
- **Path**: `/api/v1/games/match-card/level-complete`
- **Auth**: `Bearer <JWT_TOKEN>`
- **Request Body**:
  ```json
  {
...
### E. Game 2: Pattern Matching Game Telemetry & Completion

- **Method**: `POST`
- **Path**: `/api/v1/games/pattern/level-complete`
- **Auth**: `Bearer <JWT_TOKEN>`
- **Request Body**:
  ```json
  {
...
- **Response `200 OK`**:
  ```json
  {
    ...
    "monthlyLeagueXp": 30,
    "leagueTier": "Bronze Division",
    "highestUnlockedPatternLevel": 5
  }
  ```

---

### F. Daily Care Reminders (Caregiver CRUD)

- **`GET /caregiver/patient/{patientId}/reminders`**: Retrieve all reminders for linked patient.
- **`POST /caregiver/patient/{patientId}/reminders`**: `{ "title": "Blood Pressure Medicine", "time": "09:00 AM", "description": "Morning pill with water" }`
- **`PATCH /caregiver/patient/{patientId}/reminders/{reminderId}/toggle`**: Toggle active / inactive state.
- **`DELETE /caregiver/patient/{patientId}/reminders/{reminderId}`**: Remove reminder (`204 No Content`).
